...
import { Box, Typography, Button, TextField, Dialog, DialogTitle, DialogContent, Grid, Card, CardContent, Chip, IconButton, CircularProgress, Alert } from '@mui/material';
import { Add, Person, Phone, Email, LocalHospital, Edit, Delete, Close } from '@mui/icons-material';
import { useDispatch, useSelector } from 'react-redux';
import { fetchPatients, addPatient, updatePatient, deletePatient } from '@/lib/store/slices/patientsSlice';
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box display="flex" gap={1} flexWrap="wrap">
            ...
            {patient.allergies && patient.allergies !== 'None' && (
              <Chip label={`Allergies: ${patient.allergies}`} size="small" color="warning" />
            )}
          </Box>
...
export default function PatientsPage() {
  ...
  const { isReceptionist } = useAuth(); // Using useAuth hook
  ...
  useEffect(() => {
    if (debouncedSearch) {
      // You can dispatch a search-specific action here if you have one
